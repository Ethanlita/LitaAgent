...
def _build_competitors(tracker_log_dir: str) -> List[Type]:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to import AS0: {exc}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Round-robin runner for Penguin/AS0/LitaH/LitaYR")
    ...
    parser.add_argument("--target-worlds", type=int, default=650, help="Target worlds (<=700 recommended)")
    parser.add_argument("--max-worlds-per-config", type=int, default=None, help="Override max worlds per config")
    parser.add_argument("--output-dir", type=str, default=None, help="Output directory")
    parser.add_argument("--parallelism", type=str, default="loky", help="parallel/serial/dask/loky")
    parser.add_argument("--forced-logs-fraction", type=float, default=0.1, help="Force log fraction")
    ...
    output_dir = Path(args.output_dir or f"tournament_history/hrl_rr_{timestamp}").resolve()
