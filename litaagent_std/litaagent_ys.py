...
class LitaAgentYS(StdSyncAgent):
        ...
        if self._is_consumer(pid) and self.im:
            ...
            if is_near_walkaway and opponent_price_is_lower:
                ...
                max_possible_qty_issue = qty_issue.max_value if isinstance(qty_issue.max_value,
                                                                           int) else proposed_outcome_qty * 2
            ...
            proposed_outcome_qty = min(new_qty,
    ...
    def _process_optional_supply_offers(
        ...
        for pid, offer in sorted_offers:
            ...
            cheap_threshold = (
                                  self._market_material_price_avg if self._market_material_price_avg > 0 else price * 1.5) * self.cheap_price_discount
            price_is_cheap = (price <= cheap_threshold)

            demand_at_t = float(self.im.get_total_insufficient(t))
            procurement_limit_for_date_t = float(
                self.im.daily_production_capacity * 0.2) if demand_at_t == 0 else demand_at_t * 1.2
    ...
    def _process_sales_offers(
        ...
        for pid, offer in offers.items():
            ...
            if (t == self.awi.current_step):
                max_possible_purchase = sum_qty_supply_offer_today + self.im.get_inventory_summary(t,
                                                                                                   MaterialType.RAW).get(
                    'estimated_available', 0) - self.im.get_production_plan(t)
            else:
                # TODO 这里只是简单将今天的总供应量乘以天数，并不是很好的做法
                max_possible_purchase = sum_qty_supply_offer_today * (
                            t - self.awi.current_step + 1) * 0.7 + self.im.get_inventory_summary(t,
                                                                                                 MaterialType.RAW).get(
                    'estimated_available', 0) - sum(
                    self.im.get_production_plan(i) for i in range(self.awi.current_step, t + 1))
