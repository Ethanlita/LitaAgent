...
class LitaAgentYSTracked(LitaAgentYR):
    """
    带有 Tracker 功能的 LitaAgentYS（支持并行模式）
    LitaAgentYS (from litaagent_ys.py) with Tracker functionality (supports parallel mode)
    
    注意：此文件中的基类 LitaAgentYR 使用 inventory_manager_ns
    Note: The base class LitaAgentYR in this file uses inventory_manager_ns
    
    使用方法 / Usage:
        1. 设置环境变量 SCML_TRACKER_LOG_DIR 为日志目录路径
           Set environment variable SCML_TRACKER_LOG_DIR to the log directory path
        2. 在比赛中使用 LitaAgentYSTracked 代替 LitaAgentYR (在 litaagent_ys.py 中)
           Use LitaAgentYSTracked instead of LitaAgentYR (in litaagent_ys.py) in tournaments
    ...
    def tracker(self):
        ...
        if self._tracker_logger is None:
            ...
            self._tracker_logger = TrackerManager.get_logger(agent_id, 'LitaAgentYS')
...
if __name__ == "__main__":
    if os.path.exists("env.test"):
        print("模块加载成功，可在竞赛框架中使用 LitaAgentYR / LitaAgentYSTracked（来自 litaagent_ys.py）。")
        # Module loaded successfully, LitaAgentYR / LitaAgentYSTracked (from litaagent_ys.py) can be used in the competition framework.
