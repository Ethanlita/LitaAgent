...
def test_raw_material_inflow_detailed(days=10):
    ...
    for day in range(0, days):
        ...
        print(f"添加原材料合同: {contract.contract_id}, 交付日期：{contract.delivery_time}, 数量：{contract.quantity}, 单价：{contract.price:.2f}")
        ...
        for inv_day in range(day, days):
            ...
            print(f"查看第{inv_day}天库存, 现有库存: {summary['current_stock']}, 平均单价: {summary['average_cost']:.2f}, 存储成本: {summary['storage_cost']:.2f}, 预估可用库存: {summary['estimated_available']}, 预估平均单价: {summary['estimated_average_cost']:.2f}")
        # 获取各批次详细
        batch_details = im.get_batch_details(day, MaterialType.RAW)
        print(f"第 {day} 天的批次详细信息:")
        for batch in batch_details:
            print(f"批次 ID: {batch['batch_id']}, 数量: {batch['quantity']}, 单价: {batch['base_unit_cost']:.2f}, 现单价: {batch['current_unit_cost']},存储天数: {batch['storage_days']}, 存储成本: {batch['storage_cost']:.2f}, 生产时间: {batch['production_time']}")
        im.update_day()  # 推进一天
