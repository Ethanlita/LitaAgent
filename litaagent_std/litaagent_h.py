    ...
    def counter_all(self, offers, states):
        ...
        for edge_needs, all_partners, issues in [
            ...
            if self.is_supplier(all_partners[0]):
                ...
                if awi.total_sales_at(awi.current_step) <= awi.n_lines:  # すでに契約済みの量が一日に可能な生産量を下回る場合
                    needs = int(
                        max(
                            0,
                            min(
                                self.awi.n_lines,
                                day_production + awi.current_inventory_output,
                            )
    ...
    def future_consume_offer(self, list):
        ...
        if (
            ...
            step1_needs = int(
                max(
                    0,
                    min(self.awi.n_lines, p + awi.current_inventory_output)
        ...
        # 3step先で売る量
        if s + 3 < n and awi.total_sales_at(s + 3) <= self.awi.n_lines:
            step3_needs = int(
                max(
                    0,
                    min(self.awi.n_lines, p + awi.current_inventory_output)
...
if __name__ == "__main__":
    ...
    run([LitaAgentH], sys.argv[1] if len(sys.argv) > 1 else "std")
