...
- `price_diff_in/out`：成交 VWAP 与活跃报价**轮次衰减加权均值**融合（权重 0.6/0.3/0.1），分别基于买入/卖出谈判和 `spot_price_in/out`
...
* **设计决策**：统一使用经济容量：$C_{total}[k] = n\_lines \times (T_{max} - (t+k))$。
...
$$L(\\tau) \= I\_{hand}(t) \+ \\sum\_{k=t+1}^{\\tau} Q\_{in}(k) \- Q\_{prod}(k)$$  
其中：

* $I\_{hand}(t)$：当前实际在库的**原材料库存**（`awi.current_inventory_input`，非总库存）。  
* $Q\_{in}(k)$：将在未来时刻 $k$ 到货的原材料合同总量。
* $Q\_{prod}(k)$：将在未来时刻 $k$ 被生产消耗的原料量（保守估计为 `n_lines` 满负荷）。
* **物理模型说明**：$Q_{out}(k)$ 是成品出库，不影响原材料库存，因此不包含在 $L(\tau)$ 的计算中。这是纯原材料库存轨迹模型。

**重要推论（多交期采购的共存性）**：由于 $Q_{prod}$ 的消耗已包含在 $L(τ)$ 中，在早期交期（如 $δ=0$）买入的原料会随着时间被生产线消耗，从而释放库容空间，不会永久阻塞后期交期（如 $δ=2$）的采购。这使得同一批次内多个不同交期的采购可以合理共存。
...
L3层在选择交货时间 $\\delta\_t$ 时，需要更精细的约束。我们采用“满载加工假设”构建 backlog：

$$B[0]=I_{now}, \\quad B[k+1]=\\max(0, B[k]+Q_{in}[k]-n_{lines})$$

针对特定交货日 $\\delta$ 的最大买入量 $Q_{safe}[\\delta]$ 计算如下：

$$Q_{safe}[\\delta]=\\max\\big(0, C_{total}[\\delta]-B[\\delta]-\\sum_{k=\\delta}^{H-1}Q_{in}[k]\\big)$$

这意味着：在 $\delta$ 天进货的数量，不能超过交货日之后的剩余可加工量，且必须扣除已有 backlog 与未来已承诺入库量。

**向量化实现**：
```python
backlog = compute_inventory_trajectory(I_now, Q_in, n_lines, H)  # B[δ]
