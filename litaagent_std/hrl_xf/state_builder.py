...
状态空间结构：
- x_static: (12,) 静态特征向量
- X_temporal: (H+1, 9) 时序状态张量，覆盖 δ∈{0..H}
...
class StateDict:
    ...
    Attributes:
        ...
        X_temporal: 时序状态张量，shape (H+1, 9)，覆盖 δ∈{0..H}
        x_role: 角色嵌入 (One-hot)，shape (2,)
    """
    x_static: np.ndarray      # (12,)
    X_temporal: np.ndarray    # (H+1, 9)
    x_role: np.ndarray        # (2,)
    ...
    def add_batch_dim(self) -> Dict[str, np.ndarray]:
        ...
        return {
            ...
            "X_temporal": self.X_temporal[np.newaxis, :, :], # (1, H+1, 9)
...
class StateBuilder:
    ...
    def _build_temporal(self, awi: "StdAWI") -> np.ndarray:
        ...
        | 5 | balance_proj | 资金投影 |
        | 6 | price_diff | 期货溢价（基于谈判与成交数据推断） |
        | 7 | buy_pressure | 买方压力（基于谈判与合约数据） |
        | 8 | sell_pressure | 卖方压力（基于谈判与合约数据） |
        """
        # 使用 H+1 覆盖 δ∈{0..H}
        temporal_len = self.horizon + 1
        X = np.zeros((temporal_len, 9), dtype=np.float32)
        ...
        # 用于 price_diff 计算的合约价格数据
        signed_contracts_by_delta = {}  # {delta: [(qty, price), ...]}
        ...
        for contract in signed_contracts:
            ...
            if 0 <= delta <= self.horizon:  # 包含 δ=H
                ...
                # 记录用于 price_diff 计算
                if delta not in signed_contracts_by_delta:
                    signed_contracts_by_delta[delta] = []
                signed_contracts_by_delta[delta].append((quantity, unit_price))
        ...
        if trading_prices is not None and hasattr(trading_prices, '__getitem__'):
            try:
                spot_price = float(trading_prices[input_product]) if input_product < len(trading_prices) else 10.0
            except (IndexError, TypeError):
                spot_price = 10.0
        else:
            spot_price = 10.0
        
        # 计算通道 6-8：使用新的数据驱动方法（价格加权）
        # buy_pressure: 基于卖出谈判请求 + 已签买入合约，高价买单权重大
        buy_pressure = self._compute_buy_pressure(awi, Q_in, C_total, spot_price)
        
        # sell_pressure: 基于买入谈判报价 + 已签卖出合约，低价卖单权重大
        sell_pressure = self._compute_sell_pressure(awi, Q_out, C_total, spot_price)
        
        # price_diff: 基于成交 VWAP 和谈判中位价
        price_diff = self._compute_price_diff(awi, signed_contracts_by_delta, spot_price, temporal_len)
        ...
        X[:, 6] = np.clip(price_diff / max(self.max_price, 1.0), -1.0, 1.0)
        X[:, 7] = buy_pressure  # 已经是 [0, 1] 范围
        X[:, 8] = sell_pressure  # 已经是 [0, 1] 范围
    ...
    def _compute_buy_pressure(
        ...
        含义：第 t+k 天买方对商品的需求强度。
        值越大表示"买方多、缺货风险高、可抬价"。
        
        数据来源：
            1. current_sell_offers: StdAWI 提供的当前卖出谈判最新出价
               格式为 dict[partner_id, Outcome]，Outcome = (quantity, time, unit_price)
            2. 若 current_sell_offers 不可用，退化为仅基于已签合约计算
        ...
        Args:
            awi: 代理世界接口
            signed_buy_by_delta: 已签买入合约按 delta 聚合的数量向量
            capacity: 经济容量向量
            spot_price: 当前现货价格（用于价格加权）
        ...
        t_current = awi.current_step
        weighted_demand = signed_buy_by_delta.copy().astype(np.float32)
        
        # 从当前卖出谈判中获取活跃出价
        # （我方作为卖方，收到买方请求 -> 代表买方需求）
        # 高价买单权重更大
        ...
        for partner_id, offer in current_sell_offers.items():
            ...
            try:
                ...
                if 0 <= delta < temporal_len:
                    # 价格加权：高于现货价的买单权重 > 1
                    price_weight = max(0.5, min(2.0, unit_price / max(spot_price, 1.0)))
    ...
    def _compute_sell_pressure(
        ...
        数据来源：
            1. current_buy_offers: StdAWI 提供的当前买入谈判最新出价
               格式为 dict[partner_id, Outcome]，Outcome = (quantity, time, unit_price)
            2. 若 current_buy_offers 不可用，退化为仅基于已签合约计算
        ...
        Args:
            awi: 代理世界接口
            signed_sell_by_delta: 已签卖出合约按 delta 聚合的数量向量
            capacity: 经济容量向量
            spot_price: 当前现货价格（用于价格加权）
        ...
        t_current = awi.current_step
        weighted_supply = signed_sell_by_delta.copy().astype(np.float32)
        
        # 从当前买入谈判中获取活跃出价
        # （我方作为买方，收到卖方报价 -> 代表卖方供给）
        # 低价卖单权重更大
        ...
        for partner_id, offer in current_buy_offers.items():
            ...
            try:
                ...
                if 0 <= delta < temporal_len:
                    # 价格加权：低于现货价的卖单权重 > 1
                    # 使用倒数关系：价格越低，权重越高
                    price_ratio = max(spot_price, 1.0) / max(unit_price, 1.0)
    ...
    def _compute_price_diff(
        self, 
        awi: "StdAWI",
        signed_contracts_by_delta: dict,
        spot_price: float,
        ...
        数据来源：
            1. current_buy_offers/current_sell_offers: StdAWI 提供的谈判最新出价
               格式为 dict[partner_id, Outcome]，Outcome = (quantity, time, unit_price)
            2. 若这些字段不可用，price_diff 将仅基于已签合约 VWAP 计算
            3. 若无任何数据，回退到 0（即 P_future = P_spot）
            
        备选方案（如需更详细的谈判历史）：
            可从 awi.current_negotiation_details["buy"/"sell"] 获取 NegotiationDetails，
            通过 details.nmi.state.current_offer 访问出价
        ...
        Returns:
            np.ndarray: 形状 (temporal_len,) 的价格差向量
            
        TODO:
            - 权重自适应：根据样本数量动态调整 W_SIGNED/W_ACTIVE 权重
            - 添加 EMA 历史平滑以减少单笔异常出价的影响
            - 缺失 δ 可考虑插值或延用邻近 δ 的值
        """
        ...
        # 健壮地获取谈判数据
        # 注意：若这些字段不可用，price_diff 将仅基于已签合约计算
        try:
            current_buy_offers = getattr(awi, 'current_buy_offers', None)
            if current_buy_offers is None:
                current_buy_offers = {}
