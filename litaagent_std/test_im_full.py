...
def test_inventory_manager_10_days():
    ...
    for day in range(1, 11):
        ...
        for i in range(num_supply_contracts):
            ...
            if im.add_transaction(contract):
                ...
                print(f"添加供货协议: ID={contract_id}, 数量={quantity}, 价格={price:.2f}, 交货时间={delivery_time}")
            else:
                print(f"供货协议添加失败: ID={contract_id}")
        ...
        for i in range(num_demand_contracts):
            ...
            if im.add_transaction(contract):
                ...
                print(f"添加销售协议: ID={contract_id}, 数量={quantity}, 价格={price:.2f}, 交货时间={delivery_time}")
            else:
                print(f"销售协议添加失败: ID={contract_id}")

        # 3. 接收今天到货的材料
        received_materials = im.receive_materials()
        print(f"接收材料: {received_materials}")
        ...
        print(f"生产计划: {planned_str if planned_str else '无计划生产'}")
        ...
        if insufficient:
            ...
            for d, vals in insufficient.items():
                if d >= day and d <= day + 5:  # 只显示近期的不足
                    print(f"  第{d}天 - 当日必须: {vals['daily']}, 总计需要: {vals['total']}")
        ...
        print(f"交付产品: {delivered_products}")
    ...
    # 打印最终摘要
    print("\n======== 10天模拟总结 ========")
    print(f"供货协议总数: {len(supply_contracts)}")
    print(f"销售协议总数: {len(demand_contracts)}")
    ...
    print(f"最终原材料库存: {raw_summary['current_stock']} 单位，平均成本: {raw_summary['average_cost']:.2f}")
    print(f"最终产品库存: {product_summary['current_stock']} 单位，平均成本: {product_summary['average_cost']:.2f}")
    ...
    print(f"未交付原材料订单: {len(pending_supply)} 个")
    print(f"未交付产品订单: {len(pending_demand)} 个")
