...
class LitaAgentY(StdSyncAgent):
    ...
    def _update_dynamic_stockpiling_parameters(self) -> None:
        ...
        elif os.path.exists("env.test"):  # Log even if not changed, for transparency / 即使未更改也记录日志，以提高透明度
            print(
    ...
    def _pareto_counter_offer(
        ...
        if self._is_consumer(pid) and self.im:  # Selling products / 销售产品
            ...
            if os.path.exists("env.test") and len(
                    reason_log) > 1:  # Log if Pareto exploration happened / 如果进行了帕累托探索则记录日志
    ...
    def on_contracts_finalized(self, signed: List[Contract], cancelled: List[Contract],
        ...
        current_day = self.awi.current_step  # For logging / 用于日志记录
        if os.path.exists("env.test"):
            print(
        ...
        for contract in cancelled:
            ...
            removed_from_im = self.im.void_negotiated_contract(contract.id)  # Tell IM to void /告知IM作废
            if os.path.exists("env.test"):
                if removed_from_im:
                    print(f"✅ [{current_day}] ({self.id}) IM voided cancelled contract {contract.id}.")
                else:  # Should not happen if contract was previously added via on_negotiation_success / 如果合同先前已通过on_negotiation_success添加，则不应发生
                    print(
    ...
    def sign_all_contracts(self, contracts: List[Contract]) -> List[Contract]:
        ...
        if self.im is None:  # IM not available, sign all as a fallback / IM不可用，作为回退签署所有合同
            ...
            for contract_obj_iter in contracts:  # Log details if signing blindly / 如果盲目签署则记录详细信息
        ...
        if os.path.exists("env.test"):  # Added logging from Step 11 / 从步骤11添加日志
            print(
                ...
                for linked_p_info_commit in temp_linked_procurements_for_this_sale:
                    for p_master_item in available_procurement_contracts_data:
                        if p_master_item["contract_obj"].id == linked_p_info_commit["contract_obj"].id:
                            ...
                            break
            else:  # Not enough materials / 材料不足
                if os.path.exists("env.test"): print(
        ...
        while True:
            ...
            descoped_sale_data = current_selected_sales_data.pop(0)  # Remove least profitable / 移除利润最低的
            if os.path.exists("env.test"): print(
        ...
        final_total_profit_estimate = sum(
            s_d["profit"] for s_d in current_selected_sales_data)  # Estimated profit from signed sales / 已签署销售的预估利润
    ...
    def _update_dynamic_profit_margin_parameters(self) -> None:
        ...
        reason_parts = [f"Base: {new_min_profit_margin:.3f}"]  # For logging reasons for change / 用于记录更改原因
        ...
        if future_total_product_demand > 0 and current_product_inventory < future_total_product_demand * 0.5:  # Low inventory vs demand / 库存相对于需求较低
            ...
            if abs(new_min_profit_margin - initial_margin_after_rule_a) > 1e-5: reason_parts.append(
        ...
        if not rule_b_applied and abs(
                initial_margin_after_rule_a - new_min_profit_margin) < 1e-5:  # If Rule B didn't change anything / 如果规则B没有改变任何东西
        ...
        if current_day > total_days * 0.85:  # Last 15% of game / 游戏的最后15%
            ...
            if abs(new_min_profit_margin - initial_margin_after_rule_b) > 1e-5: reason_parts.append(
        ...
        if not rule_c_applied and abs(
                initial_margin_after_rule_b - new_min_profit_margin) < 1e-5:  # If Rule C didn't change anything / 如果规则C没有改变任何东西
            pass  # No specific log needed if no change / 如果没有变化则不需要特定日志
        ...
        if abs(final_new_min_profit_margin - new_min_profit_margin) > 1e-5:  # Log if clamped / 如果被限制则记录日志
    ...
    def _print_daily_status_report(self, result) -> None:
        ...
        if not self.im or not os.path.exists(
                "env.test"):  # Only print if IM exists and in test environment / 仅当IM存在且在测试环境中时打印
            return

        current_day = self.awi.current_step
        horizon_days = min(10,
                           self.awi.n_steps - current_day)  # Forecast for next 10 days or remaining days / 预测未来10天或剩余天数
        ...
        separator = "|" + "-" * (len(header) + 24) + "|"  # Adjust separator length based on content / 根据内容调整分隔符长度
        ...
        for day_offset in range(horizon_days):
            ...
            # 预测日已签署的销售合同数量
            signed_sales = 0
            for contract in self.im.get_pending_contracts(is_supply=False,
                                                          day=forecast_day):  # Iterate through pending sales / 遍历待定销售
        ...
        print(separator)
        print()  # Extra newline for readability / 额外换行以提高可读性
