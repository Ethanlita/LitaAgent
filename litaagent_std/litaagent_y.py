...
class LitaAgentYTracked(LitaAgentY):
    """
    带有 Tracker 功能的 LitaAgentY（支持并行模式）
    
    使用方法：
        1. 设置环境变量 SCML_TRACKER_LOG_DIR 为日志目录路径
        2. 在比赛中使用 LitaAgentYTracked 代替 LitaAgentY
    ...
    def tracker(self):
        ...
        if self._tracker_logger is None:
            ...
            self._tracker_logger = TrackerManager.get_logger(agent_id, 'LitaAgentY')
