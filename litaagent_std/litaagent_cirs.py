...
class LitaAgentCIRSTracked(LitaAgentCIR):
    """
    带有 Tracker 功能的 LitaAgentCIR（简化版）（支持并行模式）
    LitaAgentCIR (Simplified) with Tracker functionality (supports parallel mode)
    
    使用方法 / Usage:
        1. 设置环境变量 SCML_TRACKER_LOG_DIR 为日志目录路径
           Set environment variable SCML_TRACKER_LOG_DIR to the log directory path
        2. 在比赛中使用 LitaAgentCIRSTracked 代替 LitaAgentCIR (在 litaagent_cirs.py 中)
           Use LitaAgentCIRSTracked instead of LitaAgentCIR (in litaagent_cirs.py) in tournaments
    ...
    def tracker(self):
        ...
        if self._tracker_logger is None:
            ...
            self._tracker_logger = TrackerManager.get_logger(agent_id, 'LitaAgentCIRS')
