...
class LitaAgentP(StdSyncAgent):
    ...
    def __init__(self, *args, threshold_factor=0.1, ptoday=0.70, productivity=0.7, **kwargs):
        ...
        self._ptoday = ptoday # Percentage of partners to negotiate with for today's needs / 为满足当日需求而与之谈判的伙伴的百分比
    ...
    def counter_all(self, offers: dict[str, Outcome | None], states: dict[str, MechanismState]) -> dict[str, SAOResponse]:
        """
        ...
        for is_selling_mode in [False, True]: # False for buying, True for selling / False 代表购买, True 代表销售
            ...
            for p_id in active_partners_this_side:
                ...
                if self.is_valid_price(offer[UNIT_PRICE], p_id):
                    ...
                    elif offer[TIME] > self.awi.current_step and offer[TIME] < self.awi.n_steps: # Valid future offer / 有效的未来报价
                    ...
                    counter_offer = self._create_offer(offer[QUANTITY], offer[TIME], my_price) if my_price is not None else None
            ...
            for p_id, offer_data in future_step_offers_this_side.items():
                ...
                is_future_offer_for_my_purchase = self.is_supplier(p_id) # True if I'm buying from p_id / 如果我从 p_id 购买则为 True
                ...
                already_accepted_this_round = accepted_qty_future_step_this_round[(step, is_future_offer_for_my_purchase)]
            ...
            if needs_today > 0 and partners_for_current_combinatorics:
                ...
                best_plus_diff, best_idx_plus = float("inf"), -1 # Best combination if sum_quantity >= needs_today / 如果总数量 >= 今日需求 的最佳组合
                best_minus_diff, best_idx_minus = float("inf"), -1 # Best combination if sum_quantity < needs_today / 如果总数量 < 今日需求 的最佳组合
                ...
                if chosen_combination_idx != -1:
                    ...
                    for p_id in partner_list_powerset:
                        ...
                        if p_id in accept_partner_ids_current_step:
                            responses[p_id] = SAOResponse(ResponseType.ACCEPT_OFFER, current_step_offers_this_side[p_id])
            ...
            current_step_combo_was_accepted = (chosen_combination_idx != -1 and needs_today > 0 and accept_partner_ids_current_step)
                ...
                if partners_for_fallback:
                    ...
                    for p_id, q_assigned in dist_fallback.items():
                        ...
                        if q_assigned > 0:
                            ...
                            if self.is_supplier(p_id): fb_future_supply.append(p_id)
                            elif self.is_consumer(p_id): fb_future_consume.append(p_id)
    ...
    def is_valid_price(self, price: float | int, partner_id: str) -> bool:
        ...
        if self.is_consumer(partner_id): # Partner is my consumer (I sell to them). `price` is what they offer to pay me.
        ...
        elif self.is_supplier(partner_id): # Partner is my supplier (I buy from them). `price` is what they offer to sell at.
    ...
    def distribute_todays_needs(self, partners: Optional[list[str]] = None) -> dict[str, int]:
        ...
        supply_partners_dist: list[str] = [p for p in target_partners if self.is_supplier(p)] # I buy from these / 我从这些伙伴购买
        consume_partners_dist: list[str] = [p for p in target_partners if self.is_consumer(p)] # I sell to these / 我向这些伙伴销售
        ...
        sales_capacity_today = max(0, int(
            min(self.awi.n_lines, daily_production + self.awi.current_inventory_input) - self.awi.total_sales_at(current_step)
        ))
    ...
    def _distribute_among_subset(self, Wpartners: list[str], total_quantity: int) -> dict[str, int]:
        ...
        if total_quantity < n_selected and total_quantity > 0:
            num_final = random.randint(1, total_quantity) # Select between 1 and total_quantity partners / 选择 1 到 total_quantity 个伙伴
            selected_partners = random.sample(selected_partners, num_final) # Randomly pick from the already selected subset / 从已选子集中随机挑选
            n_selected = len(selected_partners)
    ...
    def future_supply_offer(self, partners_list: list[str]) -> dict[str, Outcome | None]:
        ...
        # 将伙伴大致分为三组用于未来步骤
        p_groups = [partners_list[:int(len_p*0.5)], partners_list[int(len_p*0.5):int(len_p*0.8)], partners_list[int(len_p*0.8):]]
        ...
        for i, step_offset in enumerate([1, 2, 3]): # For s+1, s+2, s+3 / 对于 s+1, s+2, s+3
            ...
            if future_s < n_steps and p_groups[i]: # If valid future step and partners exist for this group / 如果是有效的未来步骤且该组存在伙伴
                ...
                proposals.update(self._generate_future_offers_for_group(p_groups[i], future_s, need_for_this_group, True))
        return proposals
    ...
    def future_consume_offer(self, partners_list: list[str]) -> dict[str, Outcome | None]:
        ...
        for i, step_offset in enumerate([1, 2, 3]):
            ...
            if future_s < n_steps and p_groups[i]:
                ...
                sales_cap_at_fut_s_total = min(self.awi.n_lines, base_available_to_sell) - self.awi.total_sales_at(future_s)
                ...
                if self.awi.total_sales_at(future_s) <= self.awi.n_lines: # Keep explicit check if vital / 如果至关重要则保留显式检查
                    proposals.update(self._generate_future_offers_for_group(p_groups[i], future_s, cap_for_this_group, False))
        return proposals
