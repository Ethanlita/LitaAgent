...
class LitaAgentHS(StdSyncAgent):
    ...
    def __init__(self, *args, threshold=None, ptoday=0.70, productivity=0.7, **kwargs):
        ...
        self._l1 = L1SafetyLayer()
        self._l1_step = None
        self._l1_output = None

    def _get_l1_output(self):
        step = int(self.awi.current_step)
        if self._l1_output is None or self._l1_step != step:
            self._l1_output = self._l1.compute(self.awi, is_buying=True)
            self._l1_step = step
        return self._l1_output

    def _offer_quantity(self, offer) -> float:
        try:
            return float(offer[QUANTITY])
        except Exception:
            try:
                return float(offer[0])
            except Exception:
                return 0.0

    def _clip_offer_by_l1(self, partner, offer):
        if offer is None:
            return None
        try:
            qty, time_abs, price = offer
        except Exception:
            return offer
        try:
            time_abs = int(time_abs)
        except Exception:
            time_abs = int(self.awi.current_step)

        try:
            nmi = self.get_nmi(partner)
            time_issue = nmi.issues[TIME]
            if time_abs < time_issue.min_value or time_abs > time_issue.max_value:
                return (0, time_abs, float(price))
        except Exception:
            pass

        delta = time_abs - int(self.awi.current_step)
        if delta < 0 or delta > self._l1.horizon:
            return (0, time_abs, float(price))

        l1 = self._get_l1_output()
        if self.is_supplier(partner):
            q_new, p_new, d_new = self._l1.clip_action(
                (float(qty), float(price), int(delta)),
                Q_safe=l1.Q_safe,
                B_free=l1.B_free,
                is_buying=True,
                Q_safe_sell=l1.Q_safe_sell,
            )
        else:
            q_new, p_new, d_new = self._l1.clip_action(
                (float(qty), float(price), int(delta)),
        ...
        time_new = int(self.awi.current_step) + int(d_new)
        return (int(q_new), time_new, float(p_new))

    def _apply_l1_clips(self, response):
        for partner, resp in list(response.items()):
            if not isinstance(resp, SAOResponse):
                continue
            if resp.response == ResponseType.ACCEPT_OFFER:
                offer = resp.outcome
                if offer is None:
                    continue
                clipped = self._clip_offer_by_l1(partner, offer)
                if clipped is None:
                    continue
                if self._offer_quantity(clipped) < self._offer_quantity(offer):
                    response[partner] = SAOResponse(ResponseType.REJECT_OFFER, clipped)
                else:
                    response[partner] = SAOResponse(ResponseType.ACCEPT_OFFER, offer)
            elif resp.response == ResponseType.REJECT_OFFER:
                offer = resp.outcome
                if offer is None:
                    continue
                clipped = self._clip_offer_by_l1(partner, offer)
    ...
    def first_proposals(self):
        ...
        for k, offer in list(response.items()):
            response[k] = self._clip_offer_by_l1(k, offer)
        return response
    ...
    def counter_all(self, offers, states):
        ...
        response = self._apply_l1_clips(response)
        return response
...
if __name__ == "__main__":
    ...
    run([LitaAgentHS], sys.argv[1] if len(sys.argv) > 1 else "std")
