...
class HeuristicSettings:
    ...
    emergency_overprocurement_factor: float = 1.0  # Renamed for clarity if needed, but current name is fine
...
class LitaAgentYR(StdSyncAgent):
    ...
    def init(self) -> None:
        ...
        if os.path.exists("env.test"):  # Added from Step 11 / 从步骤11添加
            print(f"🤖 LitaAgentYR {self.id} IM initialized. Daily Capacity: {self.im.daily_production_capacity}")
...
class LitaAgentYRTracked(LitaAgentYR):
    """
    带有 Tracker 功能的 LitaAgentYR（支持并行模式）
    LitaAgentYR with Tracker functionality (supports parallel mode)
    
    使用方法 / Usage:
        1. 设置环境变量 SCML_TRACKER_LOG_DIR 为日志目录路径
           Set environment variable SCML_TRACKER_LOG_DIR to the log directory path
        2. 在比赛中使用 LitaAgentYRTracked 代替 LitaAgentYR
           Use LitaAgentYRTracked instead of LitaAgentYR in tournaments
    ...
    def tracker(self):
        ...
        if self._tracker_logger is None:
            ...
            self._tracker_logger = TrackerManager.get_logger(agent_id, 'LitaAgentYR')
...
if __name__ == "__main__":
    if os.path.exists("env.test"):
        print("模块加载成功，可在竞赛框架中使用 LitaAgentYR / LitaAgentYRTracked。")
        # Module loaded successfully, LitaAgentYR / LitaAgentYRTracked can be used in the competition framework.
