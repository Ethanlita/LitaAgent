...
使用方法：
    python examples/run_oneshot_example.py
    python examples/run_oneshot_example.py --n-steps 30
    python examples/run_oneshot_example.py --verbose
"""
...
try:
    ...
    TOP_AGENTS_2024 = get_agents(2024, as_class=True, top_only=True, track='oneshot')
except ImportError:
...
def run_oneshot_tournament(n_steps: int = 20, output_dir: str = None, verbose: bool = False,
    ...
    if output_dir is None:
        ...
        output_dir = str(ROOT_DIR / "results" / f"oneshot_{timestamp}")
    
    os.makedirs(output_dir, exist_ok=True)
    
    print("=" * 60)
    print("🎮 SCML OneShot 比赛示例")
    print("=" * 60)
    print(f"  • 步数: {n_steps}")
    ...
    lita_agents = [LitaAgentY, LitaAgentYR, LitaAgentN]
    ...
    # Top Agents
    top_agents = list(TOP_AGENTS_2025) + list(TOP_AGENTS_2024)
    
    if top_agents:
        print("  Top Agents:")
        for agent_cls in top_agents[:3]:  # 只显示前3个
            print(f"    - {agent_cls.__name__}")
...
def main():
    parser = argparse.ArgumentParser(
        description="运行 SCML OneShot 比赛示例"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="输出目录 (默认: results/oneshot_<timestamp>)"
    )
    parser.add_argument(
