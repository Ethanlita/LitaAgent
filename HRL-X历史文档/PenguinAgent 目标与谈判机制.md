...
L4 不再直接接收 L3 的隐状态（`hidden_state/latent`），而是接收**可离线重建的显式线程特征集合** `thread_feat_set` 与**全局上下文** `global_feat`。  
假设当前有 $K$ 个活跃谈判，每个线程有一条特征向量 $x\_k$（交期/角色、谈判进度、报价偏差、该交期的 `X_temporal[δ]` 切片、L1 baseline/约束等），L4 的输入是集合张量 $X\_{in} = \\{x\_1, \\dots, x\_K\\}$。  
此外，`global_feat`（对应 $C\_{global}$）包含当前的 L2 目标（或 `goal_hat`）、`x_static`、活跃线程数、资金/库存等全局信息。这样启发式 L4 与神经 L4 可共享同一输入分布，蒸馏与后续在线训练更稳定。
...
1. **Query-Key-Value 投影**：  
   * **Query ($Q$)**：来自全局上下文 $C\_{global}$（`global_feat`）。代表“当前系统最急需解决的问题是什么？”（例如：急需补货，不管价格）。  
   * **Keys ($K$)**：来自各线程特征 $x\_k$（`thread_feat`）。代表“该谈判线程当前的状况特征”（例如：报价接近目标、对手让步很快、交期紧迫）。  
   * **Values ($V$)**：同样来自 $x\_k$。  
2. 注意力权重计算：
...
3. **日间 (Negotiation Loop)**：  
   ...
   * **实时协调**：**L4 全局协调层**（Attention Network）实时汇总 `thread_feat_set`，统一计算并缓存连续权重 $\\alpha$。  
...
4. **日暮 (Settlement)**：  
   ...
   * **反馈**：当前实现以离线训练（L2/L3）+ 启发式蒸馏（L4）为主；在线联合训练（如 PPO/MAPPO）属于后续工作。
