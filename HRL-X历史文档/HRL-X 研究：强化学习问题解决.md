...
* **输入（与当前实现对齐）**：所有活跃谈判线程 $k$ 的线程显式特征 $x\_k$（`thread_feat`，可离线重建，来自可观测状态与谈判历史）以及全局上下文 $S\_{global}$（`global_feat`）。  
* 机制：协调器计算每个线程的线程权重 $\\alpha\_k$（softmax 归一化）。

  $$\\alpha \= \\text{Softmax}(\\frac{Q K^T}{\\sqrt{d\_k}})$$

  其中 $Q$ 来自全局上下文（查询向量），$K$ 来自各线程特征（键向量）。  
