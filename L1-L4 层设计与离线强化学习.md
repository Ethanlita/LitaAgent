...
**输入**：L4 接收所有活跃 L3 线程的隐状态向量集合 $\\{h\_1, h\_2, \\dots, h\_K\\}$。其中 $h\_k$ 是第 $k$ 个线程的 Transformer 输出，包含了该对手的报价历史特征（如对手是否急迫、价格是否优惠）。
...
* **高权重（$\\alpha\_k \\to 1$）**：意味着该线程的对手（例如报价极低的供应商）对实现 L2 的目标至关重要。L4 会向该线程的 L3 发送增强信号，允许其更加激进地成交（例如允许略微突破 L2 的限价，或优先响应）。**这就是“选择了该对象”。**  
...
2. **第 5 轮**：供应商 C 突然大幅降价。线程 C 的 $h\_C$ 发生变化。  
...
| **L4** | thread\_embeds | $(B, K=50, 64)$ |  | L3 输出 |
...
| **L4** | attn\_weights | Softmax Dist | 各线程资源分配权重 | 标量乘法调制 L3 输出 |
...
| **L4** | 轮 (Round) | Attention Network | 线程隐状态 $h\_k$ | Weights $\\alpha\_k$ | 端到端随 L3 训练 | 全局上下文 |
